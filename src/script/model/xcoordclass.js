...
var XCoord = function(xinit, graph) {
  ...
  if (xinit) {
    this.xcoord = xinit;
  } // coordinates of _center_ of every vertex
  else {
...
XCoord.prototype = {
  ...
  relationshipOrChhub: function(v) {
    if (this.graph.GG.type[v] == BaseGraph.TYPE.RELATIONSHIP || this.graph.GG.type[v] == BaseGraph.TYPE.CHILDHUB) {
      return true;
    }
    return false;
  },
  ...
  isBettertThan: function(otherScore) {
    if (this.score == otherScore.score) {
      if (this.numStraightLong == otherScore.numStraightLong) {
        ...
        if (score1 == score2) {
          return (Math.max.apply(null,this.inEdgeMaxLen) < Math.max.apply(null,otherScore.inEdgeMaxLen));
        } // given everything else equal, prefer layout with shorter longest edge
...
export { XCoord, XCoordScore, VerticalLevels };
